...
// statically (exactly how Vercel serves it — SPA fallback to index.html for
// every router-owned path, with the real vercel.json headers applied).
...
//
// Only PUBLIC routes are auditable: everything under /app and /present sits
// behind authGuard and redirects to the login page without a session, so
// auditing them would silently measure the login page under another name.
// See scripts/routes.mjs.
//
// Usage:
//   npm run lighthouse                 # representative sample (fast)
//   npm run lighthouse -- --all        # every URL in sitemap.xml, both
//                                       # forced color schemes
//   npm run lighthouse -- --routes=/auth/login,/student/join
...
// Lighthouse default. Default stays mobile-only.
...
// both — pass `--themes=dark` to halve audit count.
const themes = args.find((a) => a.startsWith('--themes='))?.split('=')[1]?.split(',') ?? [
  'dark',
  'light',
];
...
// The public surface is small enough that the "sample" is everything public.
const REPRESENTATIVE_SAMPLE = PUBLIC_ROUTES.flatMap((r) =>
  themes.map((theme) => ({ path: r.path, theme })),
);

function routesFromSitemap() {
  const xml = readFileSync(path.join(browserDir, 'sitemap.xml'), 'utf8');
...
// is CSP/header-blind and a real production violation (e.g. a script hash
// Vercel's browser-side CSP rejects) silently passes locally. Loaded once
// at startup so a change to vercel.json is picked up per run.
...
// production.
//
// Both the file read and its compressed forms are cached per path. A real
// CDN compresses each asset ONCE at the edge and serves the stored result;
// compressing on every request instead (brotli at its default quality 11 is
// especially slow on a ~300kB bundle) makes this server, not the app, the
// slowest thing in the measurement — it showed up as ~950ms TTFB on every
// route, which then propagates straight into FCP/LCP and the Performance
// score. Quality 5 matches the level CDNs actually serve at.
...
function startStaticServer() {
  return new Promise((resolve) => {
    const server = createServer((req, res) => {
      ...
      // SPA fallback — this build is client-rendered, so every router-owned
      // path resolves to the one index.html, exactly as vercel.json rewrites.
      if (!existsSync(filePath) || statSync(filePath).isDirectory()) {
        filePath = path.join(browserDir, 'index.html');
      }
      try {
        const asset = loadAsset(filePath);
        const headers = {
  ...
  try {
    for (const route of routes) {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS && !ok; attempt += 1) {
        console.log(
          `Auditing ${route.path} (${route.theme}, ${route.device})…${attempt > 1 ? ` (attempt ${attempt})` : ''}`,
        );
        try {
          ...
          if (attempt === MAX_ATTEMPTS) {
            console.error(
              `  giving up on ${route.path} (${route.theme}, ${route.device}) after ${MAX_ATTEMPTS} attempts`,
            );
            failed.push(route);
          }
        }
