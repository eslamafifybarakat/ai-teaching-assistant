...
//
// Unlike the blueprint project, this app is client-rendered: @angular/build
// emits no server/ directory, so there is nothing to strip afterwards to keep
// the deployment fully static — it already is.
...
console.log(
  `vercel-build: VERCEL_ENV=${vercelEnv ?? '(none)'} branch=${branch || '(none)'} -> npm run ${script}`,
);
