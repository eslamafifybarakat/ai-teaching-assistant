// Scans the build output for every inline <script> (excluding inert data
// blocks like JSON-LD, which Chrome doesn't enforce script-src against) and
// inline event-handler attribute, computes their CSP hash-source values, and
// rewrites vercel.json's Content-Security-Policy `script-src` directive to
// allow exactly those — nothing more (no 'unsafe-inline').
//
...
function main() {
  ...
  const htmlFiles = walkHtmlFiles(browserDir);
  ...
  const scriptSrcParts = [`'self'`, ...[...scriptHashes].sort().map((h) => `'sha256-${h}'`)];
  ...
  if (!cspHeader) {
    console.log('No Content-Security-Policy header configured in vercel.json — nothing to update.');
    return;
  }

  const directives = cspHeader.value
    .split(';')
    .map((d) => d.trim())
    .filter(Boolean);
  const scriptSrcIndex = directives.findIndex((d) => d.startsWith('script-src'));
  if (scriptSrcIndex === -1) {
    directives.push(newScriptSrc);
  } else {
  ...
  console.log(
    `vercel.json CSP updated — script-src now allows ${scriptHashes.size} script hash(es)` +
      (handlerHashes.size
        ? ` and ${handlerHashes.size} event-handler hash(es) (via 'unsafe-hashes')`
        : '') +
      ` across ${htmlFiles.length} built page(s).`,
  );
}
